        ...
        if str(df) != 'None':
            ...
            with st.container():
                ...
                with st.form("my_form"):
                    ...
                    if submitted:
                        Reglas = apriori(TransLista, min_support=min_support, min_confidence=min_confidence, min_lift=min_lift)
                        Resultados = list(Reglas)
                        if len(Resultados) == 0:
                            st.warning("No hay reglas con los parámetros introducidos")
                        else:
